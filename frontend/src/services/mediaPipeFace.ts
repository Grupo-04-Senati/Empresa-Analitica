...
export interface FaceScanResult {
  ...
  confidence: number;
  timestamp: number;
  /** Numero de rostros detectados en el frame (para deteccion multi-persona). */
  faceCount: number;
}
 ...
 */
/**
 * Detecta si hay mas de un rostro en el frame.
 * Debe llamarse despues de detectFrame() o usando el mismo result crudo.
 * Retorna el numero de rostros detectados.
 */
export function countFaces(video: HTMLVideoElement, timestampMs?: number): number {
  if (!faceLandmarker) return 0;
...
export function detectFrame(video: HTMLVideoElement, timestampMs?: number): (FaceScanResult & { faceCount: number }) | null {
