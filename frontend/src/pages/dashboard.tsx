...
export const Dashboard = () => {
  ...
  const [stats, setStats] = useState<DashboardStats>({
    totalClientes: 0, totalComentarios: 0, avgTiempoAtencion: 0,
    porcentajeProcesados: 0, totalAnalisis: 0, comentariosPendientes: 0,
    misSolicitudes: 0, misPendientes: 0, misResueltas: 0,
  });
    ...
    const fetchDashboard = async () => {
      try {
        ...
        const [clientesRes, comentariosRes, tiemposRes, analisisRes] = await Promise.all([
          ...
          isAdmin
            ? supabase.from('comentarios').select('id, procesado, canal, estado, fecha, tipo, usuario_id, clientes(usuario_id)')
            : supabase.from('comentarios').select('id, procesado, canal, estado, fecha, tipo, usuario_id, clientes(usuario_id)').eq('tipo', 'comentario'),
          isAdmin
        ...
        let allComentarios = comentariosRes.data || [];

        if (!isAdmin && user?.id) {
          allComentarios = allComentarios.filter((c: any) =>
            c.usuario_id === Number(user.id) || c.clientes?.usuario_id === Number(user.id)
          );
        }
        ...
        const misSolicitudes = allComentarios.filter((c: any) => c.tipo === 'solicitud').length;
        const misPendientes = allComentarios.filter((c: any) => c.tipo === 'solicitud' && c.estado === 'pendiente').length;
        const misResueltas = allComentarios.filter((c: any) => c.tipo === 'solicitud' && c.estado === 'resuelto').length;
