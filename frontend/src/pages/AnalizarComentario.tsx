...
export const AnalizarComentario = () => {
  ...
  const cargarRecientes = async () => {
    const { data } = await supabase
      .from('comentarios')
      .select('id, contenido, canal, fecha, analisis_nlp(categoria_detectada, confianza, palabras_frecuentes, sentimiento)')
      .eq('tipo', 'comentario')
      .order('fecha', { ascending: false })
      .limit(20);
    if (!data) return;

    const rows = data as unknown as AnalisisReciente[];
    const needFix = rows.filter(r => r.analisis_nlp && r.analisis_nlp.confianza == null && r.contenido);
    if (needFix.length === 0) {
      setRecientes(rows);
      return;
    }

    const results = await Promise.all(needFix.map(async (r) => {
      const result = await analizar(r.contenido);
      await supabase.from('analisis_nlp').update({
        categoria_detectada: result.categoria_detectada,
        confianza: result.confianza / 100,
      }).eq('comentario_id', r.id);
      return { id: r.id, categoria_detectada: result.categoria_detectada, confianza: result.confianza / 100, sentimiento: result.sentimiento, palabras_frecuentes: result.palabras_frecuentes.map(p => p.palabra) };
    }));
    const fixMap = new Map(results.map(r => [r.id, r]));
    const enriched = rows.map(r => {
      const fix = fixMap.get(r.id);
      if (fix) return { ...r, analisis_nlp: { ...r.analisis_nlp!, categoria_detectada: fix.categoria_detectada, confianza: fix.confianza, sentimiento: fix.sentimiento, palabras_frecuentes: fix.palabras_frecuentes } };
      return r;
    });
    setRecientes(enriched);
  };
