...
export const Clasificacion = () => {
  ...
  const fetchData = async () => {
    ...
    if (comRes.data) {
      const rows = comRes.data as unknown as ClasRow[];
      const needFix = rows.filter(c => c.analisis_nlp && c.analisis_nlp.confianza == null && c.contenido);
      if (needFix.length > 0) {
        const results = await Promise.all(needFix.map(async (c) => {
          const result = await analizar(c.contenido);
          ...
          return { id: c.id, categoria_detectada: result.categoria_detectada, confianza: result.confianza / 100 };
        }));
        const fixMap = new Map(results.map(r => [r.id, r]));
        const enriched = rows.map(c => {
          const fix = fixMap.get(c.id);
          if (fix) return { ...c, analisis_nlp: { ...c.analisis_nlp!, categoria_detectada: fix.categoria_detectada, confianza: fix.confianza } };
          return c;
        });
        setComentarios(enriched as unknown as ClasRow[]);
      } else {
        setComentarios(rows);
      }
    }
