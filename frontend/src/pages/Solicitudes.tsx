...
import { ClipboardList, Plus, Search, Filter, Loader2, Clock, CheckCircle2, AlertTriangle, X, Send, Eye, UserCheck } from 'lucide-react';
...
const estadoConfig: Record<string, { label: string; cls: string; icon: typeof Clock }> = {
  ...
  cancelado: { label: 'Cancelado', cls: 'bg-slate-100 text-slate-500', icon: X },
};
...
const canalOptions = ['web', 'telefono', 'presencial', 'email'];
...
export const Solicitudes = () => {
  ...
  const actualizarEstado = async (id: number, nuevoEstado: string) => {
    const { error: err } = await supabase.from('solicitudes').update({
      estado: nuevoEstado,
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtrados.map(s => {
                  ...
                  const pri = prioridadConfig[s.prioridad || 'normal'] || prioridadConfig.normal;
                  const Icon = est.icon;
                  return (
                    <tr key={s.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                      ...
                      <td className="py-3 px-4 text-slate-500 text-xs whitespace-nowrap">{new Date(s.fecha_solicitud).toLocaleDateString('es-ES')}</td>
                      <td className="py-3 px-4 text-right">
                        <div className="flex items-center justify-end gap-1">
                          <button onClick={() => setShowDetailModal(s)} className="p-1.5 rounded-lg hover:bg-slate-50 text-slate-400 hover:text-blue-600 transition" title="Ver detalle">
                            <Eye size={14} />
                          </button>
                          {isAdmin && (
            ...
            <div className="p-5 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <span className={`inline-block px-2 py-1 rounded-full text-xs font-medium mt-1 ${(prioridadConfig[showDetailModal.prioridad || 'normal'] || prioridadConfig.normal).cls}`}>
                    {(prioridadConfig[showDetailModal.prioridad || 'normal'] || prioridadConfig.normal).label}
                  </span>
                </div>
                <div>
                ...
                <div>
                  <p className="text-xs text-slate-500 uppercase">Fecha Solicitud</p>
                  <p className="text-sm text-slate-700 mt-1">{new Date(showDetailModal.fecha_solicitud).toLocaleString('es-ES')}</p>
                </div>
                {showDetailModal.fecha_inicio && (
                  <div>
                    <p className="text-xs text-slate-500 uppercase">Fecha Inicio</p>
                    <p className="text-sm text-slate-700 mt-1">{new Date(showDetailModal.fecha_inicio).toLocaleString('es-ES')}</p>
                  </div>
                )}
                {showDetailModal.fecha_resolucion && (
                  <div>
                    <p className="text-xs text-slate-500 uppercase">Fecha Resolucion</p>
                    <p className="text-sm text-slate-700 mt-1">{new Date(showDetailModal.fecha_resolucion).toLocaleString('es-ES')}</p>
                  </div>
                )}
                {showDetailModal.operador && (
                  <div>
                    <p className="text-xs text-slate-500 uppercase">Operador</p>
                    <p className="text-sm text-slate-700 mt-1">{showDetailModal.operador}</p>
                  </div>
                )}
              </div>
              <div>
