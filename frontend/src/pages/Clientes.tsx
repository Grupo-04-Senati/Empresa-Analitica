...
export const Clientes = ({ autoOpen = false }: { autoOpen?: boolean }) => {
  ...
  useEffect(() => {
    if (autoOpen || searchParams.get('nuevo') === '1') {
      openCreate();
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editando) {
        const { error: err } = await supabase.from('clientes').update(form).eq('id', editando.id);
        if (err) throw err;
      } else {
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('¿Eliminar este cliente permanentemente? Esta acción borrará el registro de la base de datos.')) return;
    setError('');
    try {
      // Disassociate referenced comments & service times first to prevent foreign key errors
  ...
  const handleToggleActive = async (c: ClienteDB) => {
    ...
    setError('');
    try {
      const { error: err } = await supabase.from('clientes').update({ activo: nuevoEstado }).eq('id', c.id);
      if (err) throw err;
      fetchClientes();
    } catch (err) {
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">Gestión y seguimiento de la cartera de clientes</p>
          </div>
          {canEdit && (
          <button
            onClick={openCreate}
        ...
        <div className="rounded-xl bg-white shadow-sm">
          ...
          <div className="flex items-center gap-3 border-b border-slate-100 px-5 py-4">
            ...
            <select
              ...
              className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700 outline-none transition focus:border-blue-400 focus:ring-2 focus:ring-blue-100"
            >
