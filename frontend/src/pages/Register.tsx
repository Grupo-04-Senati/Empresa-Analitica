        ...
        if (enableFace && capturedFacePhotos) {
          ...
          try {
            const res = await fetch('/face/register', {
              ...
              body: JSON.stringify({ usuario_id: result.userId, frontal: capturedFacePhotos.frontal, izquierda: capturedFacePhotos.izquierda, derecha: capturedFacePhotos.derecha }),
            });
            const data = await res.json();
            console.log('[Register] Face response:', res.status, data);
            if (!res.ok) {
              setSuccessMsg('Cuenta creada pero el rostro fallo: ' + (data.detail || 'error'));
            ...
            setSuccessMsg('Cuenta creada pero error de conexion con servidor facial: ' + faceErr.message);
            setTimeout(() => navigate('/login'), 3000);
            return;
          }
        } else {
