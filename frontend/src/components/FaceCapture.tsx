  ...
  const doLogin = useCallback(async (photos: Record<string, string>) => {
    ...
    try {
      const res = await fetch(`${API_URL}/face/login`, {
        ...
        body: JSON.stringify({ frontal: photos.frontal, izquierda: photos.izquierda || null, derecha: photos.derecha || null }),
      });
      const data = await res.json();
      if (!res.ok) { setErrorMsg(data.detail || 'Rostro no reconocido'); setPhase('error'); return; }
      setSuccessMsg(`Bienvenido ${data.nombre}`);
      setPhase('done');
      stopAll();
      if (onLoginMatch) onLoginMatch(data.usuario_id, data.nombre);
    } catch {
