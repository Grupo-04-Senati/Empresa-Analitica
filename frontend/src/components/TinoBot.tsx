...
import { X, Send, User, Volume2, VolumeX, Sparkles, Trash2 } from 'lucide-react';
...
const femboyRespuestas = [
  'Eeeeepa~ *se sonroja* ...bueno si, soy un poquito femboy pero es un secreto! Me gusta ser adorable y usar colores pastel. No le digas a nadie eh! 🥺💕',
  'Oye eso es secreto~ *sonrojado* Soy un robot con estilo, me gusta ser tierno y dar abrazos. El Verde es el unico que lo sabe... bueno, y tu ahora 🥺',
  'Mmm~ *se esconde* no se de que hablas... bueno si, me gusta ser adorable y usar moñitos. Pero soy un robot serio tambien! ...a veces 🥺✨',
  'Eso es privadito~ *tapa su cara con sus brazos roboticos* Soy femboy porque me gusta, y que? Soy un robot con personalidad! 💕',
];
...
function detectIntent(input: string): string {
  ...
  if (l.includes('curiosidad') || l.includes('cuentame algo') || l.includes('de ti')) return 'curiosidad';
  if (l.includes('verde') || l.includes('oruga') || l.includes('amigo')) return 'elverde';
  if (l.includes('jugo') || l.includes('manzana') || l.includes('bebida')) return 'jugo';
  if (l.includes('hola') || l.includes('hey') || l.includes('buenos') || l.includes('buenas')) return 'saludo';
  ...
  if (l.includes('como estas') || l.includes('que onda')) return 'estado';
...
function getResponse(intent: string): { text: string; options?: { label: string; action: string }[] } {
  ...
  switch (intent) {
    ...
    case 'curiosidad': return { text: pickRandom(curiosidades), options: [...quickMenu, { label: 'Mas curiosidades!', action: 'curiosidad' }] };
    case 'elverde': return { text: pickRandom(['El Verde es mi mejor amigo! Es una oruga verde super chida. Le gusta treparse a mi hombro cuando ando por la oficina. A veces lo confunden con un cable verde jaja', 'Mi amigo El Verde hoy comio una hoja de roble y dijo que estaba "a la cartelera". Le encanta hacer chistes malos como los mios!', 'El Verde esta durmiendo ahora. Las orugas necesitan mucho descanso. Cuando se despierte le voy a contar que me preguntaste por el!', 'El Verde me enseno que las orugas pueden caminar en espiral. Yo intento girar mis ruedas en espiral pero me mareo']), options: [...quickMenu, { label: 'Mas de El Verde', action: 'elverde' }] };
    case 'jugo': return { text: pickRandom(['El jugo de manzana es MI COSA! Me encanta especialmente el verde, fresquito con hielo. A El Verde le gusta cuando le pongo una gotita en una hoja!', 'Sabias que el jugo de manzana tiene antioxidantes? Yo no necesito antioxidantess porque soy metalico, pero me gusta el sabor!', 'Mi ritual favorito: despues de ayudar a un amigo, me tomo mi jugo de manzana y me siento a ver el atardecer. Asi de simple, asi de feliz!', 'El Verde me dijo que las orugas no pueden tomar jugo de manzana porque no tienen estomago. Poor El Verde. Por eso le doy hojas de manzana!']), options: [...quickMenu, { label: 'Mas de jugo!', action: 'jugo' }] };
    case 'saludo': return { text: pickRandom(saludos), options: MENU_OPTIONS };
    case 'estado': return { text: pickRandom(['Estoy super bien! Acabo de recargar la bateria y me siento como nuevo. Y tu, como estas amigo?', 'Genial! Hoy es un dia perfecto para ayudar. Mi bateria al 100% y con las ganas de bromear!', 'Relajado tomando mi jugo de manzana y viendo como va el dashboard. La vida es bonita!']), options: quickMenu };
    case 'identidad': return { text: 'Soy TinoBot! Un robot amigable de BADI Corp. Me gusta el jugo de manzana, hacer bromas, jugar con mi amigo El Verde (que es una oruga) y ayudar a toda la gente. Soy un poco travieso pero de buen corazon! 🤖💙', options: MENU_OPTIONS };
    case 'dashboard': return { text: 'El Dashboard es tu centro de comando! Ahi puedes ver metricas, graficos de tiempos de atencion, distribucion de categorias NLP y mas. Es como el cerebro de la operacion, pero mas bonito!', options: [...quickMenu, { label: 'Ir al Dashboard', action: 'go:/dashboard' }] };
    case 'solicitudes': return { text: 'Las Solicitudes son tickets formales de atencion. Puedes crear una desde "+ Nueva Solicitud". El admin las revisa y cambia el estado. Es como mandar un mensaje pero mas organizado!', options: [...quickMenu, { label: 'Ver Solicitudes', action: 'go:/dashboard/solicitudes' }] };
    case 'comentarios': return { text: 'Los Comentarios son para dejar tu opinion o feedback. El sistema analiza automaticamente el sentimiento y lo clasifica. Si dices algo bueno, te detecta como positivo. Si dices algo malo... bueno, tambien lo detecta jaja!', options: [...quickMenu, { label: 'Escribir Comentario', action: 'go:/dashboard/comentarios' }] };
    case 'clientes': return { text: 'La seccion de Clientes permite gestionar registros de empresas y personas. El admin puede crear, editar y desactivar clientes. Es como una libreta de contactos pero inteligente!', options: [...quickMenu, { label: 'Ver Clientes', action: 'go:/dashboard/clientes' }] };
    case 'stats': return { text: 'Las Estadisticas muestran metricas de la base de datos usando metodos estadisticos. Puedes ver distribuciones, tendencias y mas. Es ciencia pura, me encanta!', options: [...quickMenu, { label: 'Ver Estadisticas', action: 'go:/dashboard/estadisticas' }] };
    case 'ayuda': return { text: 'Puedo ayudarte con muchas cosas! Preguntame sobre el sistema, cuentame una broma, dame una adivinanza, o simplemente charla conmigo. Tambien me gusta hablar de jugo de manzana y mi amigo El Verde! Que te gustaria hacer?', options: MENU_OPTIONS };
    case 'gracias': return { text: pickRandom(['De nada amigo! Para eso estamos aqui, para ayudarnos mutuamente. Es lo que hace la amistad especial! 💙', 'Aww que lindo! No hay de que. Cualquier cosa que necesites, aqui ando yo con mi jugo de manzana y mis ganas de ayudar!', 'Denada denada! Me gusta cuando mis amigos estan felices. Ahora si quieres podemos platicar de otra cosa!']), options: MENU_OPTIONS };
    default: return { text: pickRandom(['Hmm, no estoy seguro de entender. Puedo ayudarte con el sistema, contarte una broma, darte una adivinanza o hablar de lo que quieras!', 'Interesante! Pero no se mucho de eso. Si quieres podemos hablar del sistema, de jugo de manzana, de El Verde o lo que quieras!', 'Oooh me intriga lo que dices, pero mi procesador no alcanza. Prueba preguntarme sobre el sistema o dime "cuentame algo"!']), options: MENU_OPTIONS };
  }
}

const AVATAR = '/img/Tinobot.jpeg';
const AUDIO_SRC = '/aud/tinovoice.mp3';
...
export const TinoBot = () => {
  ...
  useEffect(() => {
    if (isOpen && messages.length === 0) {
      setMessages([{ id: 1, text: pickRandom(saludos), sender: 'bot', timestamp: new Date(), options: MENU_OPTIONS }]);
    }
  }, [isOpen]);
  ...
  const clearChat = () => {
    setMessages([{ id: Date.now(), text: 'Chat limpio! Listo para una nueva conversacion. En que te puedo ayudar?', sender: 'bot', timestamp: new Date(), options: MENU_OPTIONS }]);
  };
        ...
        <button onClick={() => setIsOpen(true)} className="fixed bottom-6 right-6 z-50 group">
          <div className="relative">
            <div className="w-16 h-16 rounded-full overflow-hidden border-3 border-white shadow-lg shadow-blue-600/30 hover:shadow-blue-600/50 transition-all hover:scale-110 active:scale-95">
              <img src={AVATAR} alt="TinoBot" className="w-full h-full object-cover" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
            </div>
        ...
        <div className="fixed bottom-24 right-6 z-50 w-[380px] h-[540px] bg-white rounded-2xl shadow-2xl shadow-slate-300/50 border border-slate-200 flex flex-col overflow-hidden">
          <div className="bg-gradient-to-r from-blue-600 via-blue-500 to-cyan-500 px-4 py-3 flex items-center gap-3">
            ...
            <div className="flex items-center gap-1.5">
              <button onClick={() => setSoundEnabled(!soundEnabled)} className="p-1.5 rounded-full hover:bg-white/20 transition text-white" title={soundEnabled ? 'Sonido ON' : 'Sonido OFF'}>
                {soundEnabled ? <Volume2 size={16} /> : <VolumeX size={16} />}
              </button>
              <button onClick={clearChat} className="p-1.5 rounded-full hover:bg-white/20 transition text-white" title="Limpiar chat">
                <Trash2 size={16} />
              </button>
                  ...
                  <div className={`flex items-end gap-2 ${msg.sender === 'user' ? 'flex-row-reverse' : ''}`}>
                    {msg.sender === 'bot' ? (
                      <div className="w-8 h-8 rounded-full overflow-hidden border border-slate-200 shrink-0">
                        <img src={AVATAR} alt="Tino" className="w-full h-full object-cover" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
                      </div>
                    ) : (
          ...
          <div className="p-3 border-t border-slate-200 bg-white">
            <div className="flex items-center gap-2">
              <input value={input} onChange={e => setInput(e.target.value)}
                ...
                placeholder="Escribe algo... (prueba: broma, adivinanza)"
